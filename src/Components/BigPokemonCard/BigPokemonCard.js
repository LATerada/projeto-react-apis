...
const BigPokemonCard = (props) => {
    ...
    return(
        ...
        position='relative' w='86.821rem' h='41.438rem' borderRadius='2.368rem' templateColumns='repeat(4,1fr)' templateRows='repeat(4,1fr)' justifyItems='center' alignItems='center' bg={cardColor(pokemon.types[0].type.name)}>
            ...
            <GridItem 
            colStart={2} colSpan={1} rowSpan={4} w='21.438rem' h='38.313rem' bg='white' borderRadius='0.75rem' textStyle={'baseStats'} >Base stats
                {pokemon.stats.map((stat) => {
                    console.log(stat)
                    return(
                        <Box>
                            <Text textStyle={'stat'} >{stat.stat.name}</Text>
                            <Text textStyle={'stat'} >{stat.base_stat}</Text>
